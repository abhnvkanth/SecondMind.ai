...
export async function POST(req: NextRequest) {
  try {
    ...
    if (feature === "summary") {
      prompt = `You are SecondMind, an advanced AI personal assistant. Generate a concise, clear summary of this alert.
...
- Do not use markdown titles, just plain bullets.`;
      ...
      prompt = `You are SecondMind, an advanced AI personal assistant. Suggest the single best next action the user should take to resolve this alert.
...
- Be extremely practical (e.g. "Draft an update message to Sarah", "Review the budget spreadsheet", "Confirm receipt of payment").`;
      ...
      prompt = isEmail
        ? `You are SecondMind, an AI personal assistant. Draft a professional, concise email reply to the sender of this alert.
...
- Output only the email body text, no subject lines or headers.`
        : `You are SecondMind, an AI personal assistant. Draft a concise, natural message reply for this chat alert.
