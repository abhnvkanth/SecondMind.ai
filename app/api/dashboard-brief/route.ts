...
import { insforgeAdmin, hasInsforgeAdminKey } from "@/lib/insforge-admin";
  ...
  try {
    if (!hasInsforgeAdminKey) {
      return NextResponse.json(
        { error: "INSFORGE_API_KEY is required for dashboard-brief with RLS enabled." },
        { status: 503 }
      );
    }
    ...
    if (hasApiKey && hasData) {
      try {
        ...
        const systemPrompt = `You are an advanced AI personal assistant named SecondMind.
